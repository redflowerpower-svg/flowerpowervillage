...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.query.action === "cleanup") {
    try {
      ...
      async function listAllPngs(path: string = ""): Promise<string[]> {
        ...
        for (const item of (items || [])) {
          ...
          if (!item.id) {
            // It's a directory
            const subPngs = await listAllPngs(itemPath);
            pngs = pngs.concat(subPngs);
      ...
      console.log("Starting recursive scan for PNG files...");
      const allPngs = await listAllPngs("");
      
      // Filter only the categories specified by the user
      const targetFolders = [
        "02-Pasta/",
        "03-Lasagne/",
        "04-Italian-Salads/",
      ...
      const pngsToDelete = allPngs.filter(filePath => {
        const isInTargetFolder = targetFolders.some(folder => filePath.startsWith(folder));
        const isBologneseRootFile = !filePath.includes("/") && filePath.toLowerCase().endsWith("-bolognese.png");
        return isInTargetFolder || isBologneseRootFile;
      });

      console.log(`Found ${pngsToDelete.length} matching PNG files to delete:`, pngsToDelete);

      if (pngsToDelete.length === 0) {
        return res.status(200).json({ success: true, message: "No matching PNG files found to delete" });
      }
      ...
      return res.status(200).json({ success: true, message: `Successfully deleted ${pngsToDelete.length} PNG files`, deleted });
    } catch (err: any) {
