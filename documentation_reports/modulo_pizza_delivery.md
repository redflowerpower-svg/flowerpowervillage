...
*   **Supporto Bilingue Completo (Inglese & Thailandese - EN / TH):**
    *   **Tablet KDS (`KitchenTabletKDS.tsx`):** Tutte le etichette, badge, contatori di colonna (`NEW · รอรับ`, `OVEN · กำลังอบ`, `READY · พร้อมส่ง`), pulsanti di azione (`ACCEPT · ยืนยัน`, `MUTE ALARM · ปิดเสียง`, `BAKED ➔ READY FOR RIDER · อบเสร็จแล้ว ➔ พร้อมส่ง`, `DELIVERED & ARCHIVED · จัดส่งแล้ว / บันทึกประวัติ`) e comandi tablet (`SCREEN ON · เปิดจอค้าง`, `TEST 🔔 ทดสอบ`) sono completamente bilingui EN/TH.
    *   **Voci Menu & Ingredienti Extra nel KDS:** Per ogni comanda viene visualizzato sia il nome in lingua originale/inglese che il nome thailandese (`getThaiName`), prelevato sia dal payload dell'ordine che dal dizionario master `menuData.ts`, permettendo al personale di cucina thailandese di leggere istantaneamente qualsiasi pietanza senza barriere linguistiche.
    *   **Notifiche & Pulsanti Telegram (`api/_handlers/telegram.ts`, `api/_helpers/telegram.ts`):** I messaggi inoltrati al canale Telegram della pizzeria contengono tutti i dettagli dell'ordine, i recapiti (Telefono, WhatsApp, LINE) e le singole pietanze in doppia lingua EN e TH. I pulsanti interattivi inline sono bilingui (`🟢 Confirm / ยืนยัน`, `✖ Reject / ยกเลิก`, `🛵 Rider Departed / ไรเดอร์ออกแล้ว`, `🏁 Delivered / ถึงแล้ว`), e le risposte dei callback query notificano lo stato in inglese e thailandese.
