import fs from 'import fs'; // Wait, let's write normal ES6 imports
import fs from 'fs';
...
async function main() {
  try {
    console.log('========================================================================');
    console.log('🔍 DIAGNOSTICA DETTAGLIATA (V3): VERIFICA STATO TARIFFE OTA');
    console.log('========================================================================\n');
    ...
    const url = `https://api.octorate.com/connect/rest/v3/roomrates/${OCTORATE_STRUCTURE_ID}`;
    const roomsRes = await fetch(url, {
      headers: { 
        'Authorization': `Bearer ${token}`
        // NOTE: Content-Type is omitted on purpose for GET requests to prevent HTTP 400 ApiParamsRequiredException
      }
    });
