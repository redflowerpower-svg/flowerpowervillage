...
export const useRestrictionsStore = create<RestrictionsState>((set, get) => ({
  ...
  updatePlannedPeriod: (ratePlanKey, index, updatedData) => {
    ...
    if (updatedPeriods[index]) {
      ...
      localStorage.setItem('fpv_planned_restrictions_v9', JSON.stringify(updated));
    }
  },
  ...
  saveDraftBackup: () => {
    ...
    localStorage.setItem('fpv_planned_restrictions_backup_v9', JSON.stringify(current));
    localStorage.setItem('fpv_planned_restrictions_backup_time_v7', new Date().toISOString());
  },

  restoreDraftBackup: () => {
    const backup = localStorage.getItem('fpv_planned_restrictions_backup_v9');
    if (!backup) {
    ...
    try {
      ...
      localStorage.setItem('fpv_planned_restrictions_v9', backup);
  ...
  importConfig: (periods) => {
    try {
      ...
      localStorage.setItem('fpv_planned_restrictions_v9', JSON.stringify(periods));
