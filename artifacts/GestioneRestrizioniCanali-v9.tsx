...
import { 
  ...
  PlannedPeriod, 
  addDay 
} from './useRestrictionsStore-v9';


const RATE_PLAN_COLORS: Record<string, {
  bg: string;
  border: string;
  text: string;
  badgeBg: string;
  ...
  agoda_ac_7d: {
    ...
    text: 'text-lime-400',
    badgeBg: 'bg-lime-950/30',
    badgeBorder: 'border-lime-500/10',
  ...
  airbnb_ac: {
    ...
    badgeBorder: 'border-orange-500/10',
  ...
  ac_7d: {
    ...
    badgeBorder: 'border-cyan-500/10',
  ...
  ac_bnb_7d: {
    ...
    badgeBorder: 'border-yellow-500/10',
...
const PeriodCard: React.FC<PeriodCardProps> = memo(({
  ...
  const planColor = RATE_PLAN_COLORS[ratePlanKey] || { bg: 'bg-stone-950/80', border: 'border-stone-750', text: 'text-stone-400', badgeBg: 'bg-stone-950/30', badgeBorder: 'border-stone-750/30', badgeText: 'text-stone-400' };
  ...
  return (
    <div className="flex items-center gap-2 h-full">
      <div 
        className={`flex-shrink-0 rounded-2xl border p-3.5 transition-all ${
        isCircled
          ? 'ring-4 ring-yellow-400 border-yellow-400 bg-yellow-950/20 shadow-[0_0_15px_rgba(234,179,8,0.6)]'
          : `${planColor.bg} ${planColor.border}`
      }`}
        style={{ width: `${Math.max(240, cardWidth)}px` }}
      >
        <div className="grid grid-cols-2 gap-3 mb-2.5">
          <div>
            <span className="text-[10px] text-stone-300 block font-bold uppercase tracking-wide">inizio</span>
            <input 
              type="date"
              value={p.dateFrom}
          ...
          <div>
            <span className="text-[10px] text-stone-300 block font-bold uppercase tracking-wide">fine</span>
            <input 
              type="date"
              value={p.dateTo}
              onClick={(e) => e.currentTarget.showPicker()}
        ...
        <div className="flex items-center justify-between gap-1.5 border-t border-stone-900/60 pt-2.5">
          <input 
            ...
            className="w-12 text-center text-xs bg-stone-900 border border-stone-800 focus:border-violet-500 rounded px-1 py-1 ${planColor.text} font-bold [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
          />
          <span className="text-[10px] text-stone-300 font-bold tracking-wider font-sans">GIORNI ONLY CHECK OUT</span>
        </div>

        <div className="flex justify-between items-center mt-2.5 pt-2 border-t border-stone-900/40">
          <button
    ...
    <div className="p-6 bg-stone-900 border border-violet-500/20 rounded-xl shadow-2xl text-stone-100 max-w-full overflow-hidden">
      ...
      <div className="space-y-8">
        ...
        <div className="p-5 bg-stone-950/40 border border-stone-800 rounded-xl">
          ...
          <div className="flex justify-between items-center mb-6 flex-wrap gap-4 border-b border-stone-850 pb-4">
            <h3 className="text-lg font-semibold text-violet-300 flex items-center gap-2 m-0">
              👑 Tabella 1: Plancia di Pianificazione Locale (Editabile)
            </h3>
            
            {/* TOOLBAR DI SICUREZZA & BACKUP */}
            <div className="flex items-center gap-2 bg-stone-950/60 p-2 rounded-lg border border-stone-800 flex-wrap">
              <span className="text-[10px] text-stone-500 font-bold uppercase tracking-wider pl-1 mr-1">🔐 Sicurezza Bozza:</span>
              
              <button
                onClick={() => {
                  saveDraftBackup();
                  const nowStr = new Date().toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
                  alert(`💾 BOZZA SALVATA CON SUCCESSO!\nLa configurazione locale corrente è stata memorizzata come backup di sicurezza alle ore ${nowStr}.`);
                }}
                className="px-3 py-1 bg-emerald-950/40 hover:bg-emerald-900/60 text-emerald-400 border border-emerald-900/50 rounded text-xs font-semibold transition-all flex items-center gap-1.5 cursor-pointer"
                title="Salva una bozza di sicurezza locale separata (prima di sincronizzare)"
              >
                💾 Salva Bozza
              </button>
              ...
              <button
                onClick={() => {
                  const confirmRestore = window.confirm(
                    "⚠️ ATTENZIONE: Sei sicuro di voler ripristinare la bozza salvata di sicurezza?\nLa configurazione modificata corrente verrà sovrascritta con l'ultimo salvataggio."
                  );
                  if (!confirmRestore) return;
                  const res = restoreDraftBackup();
                  alert(res.message);
                }}
                className="px-3 py-1 bg-stone-850 hover:bg-stone-750 text-stone-300 border border-stone-700 rounded text-xs font-semibold transition-all flex items-center gap-1.5 cursor-pointer"
                title="Ripristina l'ultimo backup di sicurezza salvato"
              >
                📂 Ripristina Bozza
              </button>
              ...
              <button
                onClick={() => {
                  const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(plannedPeriods, null, 2));
                  const downloadAnchor = document.createElement('a');
                  const dateStr = new Date().toISOString().slice(0, 10);
                  downloadAnchor.setAttribute("href", dataStr);
                  downloadAnchor.setAttribute("download", `fpv_planned_restrictions_backup_${dateStr}.json`);
                  document.body.appendChild(downloadAnchor);
                  downloadAnchor.click();
                  downloadAnchor.remove();
                }}
                className="px-3 py-1 bg-violet-950/40 hover:bg-violet-900/60 text-violet-400 border border-violet-900/50 rounded text-xs font-semibold transition-all flex items-center gap-1.5 cursor-pointer"
                title="Scarica un file .json sul tuo PC come backup definitivo a prova di bomba"
              >
                📤 Esporta File
              </button>

              <label
                className="px-3 py-1 bg-violet-950/40 hover:bg-violet-900/60 text-violet-400 border border-violet-900/50 rounded text-xs font-semibold cursor-pointer transition-all flex items-center gap-1.5"
                title="Importa un file di backup .json salvato in precedenza sul tuo PC"
              >
                📥 Importa File
                <input
                  type="file"
                    ...
                    fileReader.onload = (event) => {
                      try {
                        ...
                        const confirmImport = window.confirm(
                          "⚠️ ATTENZIONE: Sei sicuro di voler importare questo file di configurazione?\nI periodi pianificati correnti verranno completamente sostituiti."
                        );
                        if (!confirmImport) return;
                        const res = importConfig(parsed);
                        alert(res.message);
                      } catch (err: any) {
          ...
          <div className="flex border border-stone-800/80 rounded-lg overflow-hidden bg-stone-950/20">
            ...
            <div className="w-[280px] min-w-[280px] border-r border-stone-800 bg-stone-900/40 select-none">
              ...
              <div className="divide-y divide-stone-850">
                {RATE_PLANS.map(rp => {
                  const planColor = RATE_PLAN_COLORS[rp.key] || { bg: 'bg-stone-950/80', border: 'border-stone-750', text: 'text-stone-400', badgeBg: 'bg-stone-950/30', badgeBorder: 'border-stone-750/30', badgeText: 'text-stone-400' };
                  return (
                    <div 
                      key={rp.key} 
                      className="transition-all flex flex-col justify-center border-b border-stone-850/30"
                      style={{ height: expandedPlan === rp.key ? `${126 + RESORT_ACCOMMODATIONS.length * 28 + 30}px` : '126px' }}
                    >
                    <div 
                      onClick={() => setExpandedPlan(expandedPlan === rp.key ? null : rp.key)}
                      className={`p-4 cursor-pointer hover:bg-stone-850/40 transition-colors h-full flex flex-col justify-center ${expandedPlan === rp.key ? 'bg-stone-900/40' : ''}`}
                    >\n                      <div className="font-bold text-sm font-mono flex items-center justify-between">
                        <span className={planColor.text}>{rp.label}</span>
                        <span className="text-[10px] text-stone-500 font-sans">
                          {expandedPlan === rp.key ? '▼ Nascondi' : '▲ Camere'}
                        </span>
                      </div>
                      <div className="text-[11px] text-stone-400 mt-1 font-mono">{rp.fullName}</div>
                      <div className={`text-[10px] mt-0.5 px-1.5 py-0.5 rounded w-fit border font-mono ${planColor.badgeBg} ${planColor.badgeBorder} ${planColor.badgeText}`}>
                        {rp.agencies}
                      </div>
                    </div>
              ...
              <div className="divide-y divide-stone-850">
                {RATE_PLANS.map(rp => {
                  ...
                  return (
                    <div 
                      key={rp.key} 
                      className="min-h-[126px] flex items-center relative transition-all"
                      style={{ height: expandedPlan === rp.key ? `${126 + RESORT_ACCOMMODATIONS.length * 28 + 30}px` : '126px' }}
                    >
                      {spacerWidth > 0 && (
            ...
            <div className="w-[280px] min-w-[280px] border-r border-stone-800 bg-stone-900/40 select-none">
              ...
              <div className="divide-y divide-stone-850">
                {RATE_PLANS.map(rp => {
                  const planColor = RATE_PLAN_COLORS[rp.key] || { bg: 'bg-stone-950/80', border: 'border-stone-750', text: 'text-stone-400', badgeBg: 'bg-stone-950/30', badgeBorder: 'border-stone-750/30', badgeText: 'text-stone-400' };
                  return (
                    <div key={rp.key} className="h-[126px] p-4 bg-stone-900/10 flex flex-col justify-center border-b border-stone-850/30">
                      <div className={`font-bold text-sm font-mono ${planColor.text}`}>{rp.label}</div>
                      <div className="text-[11px] text-stone-500 mt-1 font-mono">{rp.fullName}</div>
                      <div className={`text-[10px] mt-0.5 px-1.5 py-0.5 rounded w-fit border font-mono ${planColor.badgeBg} ${planColor.badgeBorder} ${planColor.badgeText}`}>🔗 {rp.agencies}</div>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="flex-1 overflow-x-auto">
              ...
              <div className="divide-y divide-stone-850">
                {RATE_PLANS.map(rp => {
                  ...
                  return (
                    <div key={rp.key} className="h-[126px] flex items-center relative">
                      {spacerWidth > 0 && (
                        <div style={{ width: `${spacerWidth}px` }} className="flex-shrink-0 h-full border-r border-stone-800/10 bg-stone-950/5" />
                      )}
                      ...
                      <div className="flex items-center gap-2 px-3 relative h-full">
                        {livePeriods.map((liveP, idx) => {
                          ...
                          const planColor = RATE_PLAN_COLORS[rp.key] || { bg: 'bg-stone-950/15', border: 'border-stone-750/20', text: 'text-stone-400', badgeBg: 'bg-stone-950/30', badgeBorder: 'border-stone-750/10', badgeText: 'text-stone-400' };
                          return (
                            <div 
                              key={liveP.id}
                              className={`flex-shrink-0 rounded-2xl border p-3.5 transition-all ${
                                isCircled 
                                  ...
                                  : `${planColor.bg} ${planColor.border} ${planColor.text}`
                              }`}
                              style={{ width: `${Math.max(240, cardWidth)}px` }}
                            >
                              ...
                              <div className="flex items-center justify-between gap-1.5 border-t border-stone-900/60 pt-2.5">
                                <div className="w-12 text-center text-xs bg-stone-900 border border-stone-800 rounded px-1 py-1 ${planColor.text} font-bold font-mono">
