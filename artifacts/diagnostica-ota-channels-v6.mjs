...
async function getCalendarDataVerbose(token, rateId, dateFrom, dateTo) {
...
async function main() {
  try {
    console.log('========================================================================');
    console.log('🔍 DIAGNOSTICA VERBOSE (V6): VERIFICA STATO TARIFFE OTA');
    console.log('========================================================================\n');
    ...
    const url = `https://api.octorate.com/connect/rest/v3/roomrates/${OCTORATE_STRUCTURE_ID}?fields=id,name`;
    ...
    if (!roomsRes.ok) {
      const errorBody = await roomsRes.text();
      console.error(`\n❌ ERRORE OCTORATE HTTP ${roomsRes.status}`);
      throw new Error(`Errore scaricamento: HTTP ${roomsRes.status}`);
    }
    ...
    if (bookingRates.length > 0) {
      const firstRate = bookingRates[0];
      console.log(`\n🧪 Test diagnostico di lettura calendario per: "${firstRate.name}" (ID: ${firstRate.id})`);
      const result = await getCalendarDataVerbose(token, firstRate.id, DATE_FROM, DATE_TO);
      if (result.ok) {
        console.log(`🟢 LETTURA OK! Giorni ricevuti: ${result.data?.days?.length || 0}`);
      } else {
        console.log(`🔴 ERRORE LETTURA: Status HTTP ${result.status}. Dettagli: ${result.error}`);
      }
    }
