...
function parsePeriods(days) {
  ...
  while (i < n) {
    ...
    // Period boundary is when stopSell changes OR when CTA pattern changes
    // In our system, open period ends with CTA days at the end
...
async function testSentinelFetch() {
  ...
  // Filter Sentinel Room items (Jungle Villa rates start with "JV" or "Jungle Villa")
  ...
  console.log(`Total Sentinel (Jungle Villa) rate products: ${sentinelItems.length}`);
  for (const item of sentinelItems) {
    console.log(`- Sentinel Rate: "${item.name}" (ID: ${item.id}), Days: ${item.days?.length}`);
  }
  ...
  console.log("\nSentinel Grid map results:");
  for (const [k, v] of Object.entries(gridMap)) {
    console.log(`\nPlan ${k}: ${v.length} periods`);
    for (const p of v) {
...
testSentinelFetch().catch(console.error);
