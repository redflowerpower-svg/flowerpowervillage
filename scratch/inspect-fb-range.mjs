...
async function inspectFBRange() {
  ...
  console.log('\n--- ALL OCTORATE IDs IN RANGE 932240 - 932275 ---');
  allItems
    .filter(item => Number(item.id) >= 932240 && Number(item.id) <= 932275)
    .sort((a, b) => Number(a.id) - Number(b.id))
    .forEach(item => console.log(`ID: ${String(item.id).padEnd(8)} ➔ Name: "${item.name}"`));
}

inspectFBRange().catch(err => console.error(err));
