...
async function directOctorateSync() {
  ...
  if (refreshToken && clientId && clientSecret) {
    try {
      ...
      if (refreshRes.ok) {
        ...
        await supabase.from('octorate_tokens').upsert({ id: 'singleton', access_token: newTokens.access_token, refresh_token: newTokens.refresh_token || refreshToken, expires_in: newTokens.expires_in, updated_at: new Date().toISOString() });
      }
    } catch (e) {
      console.warn("Token refresh warn:", e);
    }
  }
  ...
  while (hasMore && page <= 25) {
    ...
    while (pageRetries > 0 && !pageSuccess) {
      try {
        ...
        pageRetries--;
        console.warn(`Fetch page ${page} retry:`, err.message);
        if (pageRetries === 0) { hasMore = false; }
        else { await new Promise(r => setTimeout(r, 1000)); }
      }
    }
  }

  console.log(`Fetched ${bookingsData.length} total active season bookings from Octorate.`);
  ...
  Object.entries(ALL_ACCOMMODATIONS_MAP).forEach(([key, canonical]) => {
    roomBookingsMap[key] = { roomName: canonical.name, motherId: String(canonical.motherId), targetProductIds: canonical.ids, bookings: [] };
  });
  ...
  Object.entries(ALL_ACCOMMODATIONS_MAP).forEach(([key, canonical]) => {
    const rawList = roomBookingsMap[key]?.bookings || [];
    const linkedList = (canonical.linkedKeys || []).flatMap(lKey => roomBookingsMap[lKey]?.bookings || []);
    effectiveRoomBookings[key] = {
      roomName: canonical.name,
      motherId: String(canonical.motherId),
      targetProductIds: canonical.ids,
      bookings: [...rawList, ...linkedList]
    };
  });

  const updates = [];
  Object.values(effectiveRoomBookings).forEach(({ roomName, motherId, bookings: bList }) => {
    ...
    if (mergedOccupied.length === 0) {
      gaps.push({ start: todayISO, end: endISO });
    } else {
      if (mergedOccupied[0].in > todayISO) {
        gaps.push({ start: todayISO, end: mergedOccupied[0].in });
      }
      for (let i = 0; i < mergedOccupied.length - 1; i++) {
        ...
        if (prevOut < nextIn && prevOut <= endISO && nextIn >= todayISO) {
          ...
          if (effectiveStart < nextIn) gaps.push({ start: effectiveStart, end: nextIn });
        }
      }
      const lastOut = mergedOccupied[mergedOccupied.length - 1].out;
      if (lastOut < endISO) {
        const effectiveStart = lastOut < todayISO ? todayISO : lastOut;
        gaps.push({ start: effectiveStart, end: endISO });
      }
    }
      ...
      if (gapDays > 0) {
        ...
        while (cur < gapEnd) {
          ...
          if (target !== blockMinStay) {
            ...
            updates.push({ roomTypeId: String(motherId), accommodationName: roomName, dateFrom: blockStart, dateTo: blockEnd, minStay: blockMinStay });
            blockStart = cur;
            blockMinStay = target;
          }
        ...
        if (blockStart <= lastDate) {
          updates.push({ roomTypeId: String(motherId), accommodationName: roomName, dateFrom: blockStart, dateTo: lastDate, minStay: blockMinStay });
        }
      }
    });
  });

  console.log(`Calculated ${updates.length} total blocks to send to Octorate.`);

  const roomsPayload = updates.map(u => ({
    room: Number(u.roomTypeId),
    dateFrom: u.dateFrom,
    dateTo: u.dateTo,
    values: { minstay: Number(u.minStay) }
  }));
  ...
  for (let idx = 0; idx < chunks.length; idx++) {
    ...
    while (retries > 0) {
      try {
        ...
        console.log(`Batch ${idx + 1}/${chunks.length} sent -> HTTP Status: ${bulkRes.status}`);
        break;
      } catch (err) {
        retries--;
        console.warn(`Batch ${idx + 1} attempt failed, retries left: ${retries}`, err.message);
  ...
  console.log("ALL BATCHES SENT SUCCESSFULLY TO OCTORATE LIVE!");
}
