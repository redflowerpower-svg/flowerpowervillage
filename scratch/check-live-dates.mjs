...
async function checkLiveDates() {
  ...
  let allItems = [];
  while (page < 15) {
    const url = `https://api.octorate.com/connect/rest/v1/calendar/${structureId}?dateFrom=2027-01-03&dateTo=2027-01-18&size=20&page=${page}`;
  ...
  const targetRates = [421511, 495795, 529773];
  ...
  matched.forEach(r => {
    ...
    if (r.days) {
      r.days.forEach(d => {
        console.log(`   ${d.date}: minStay=${d.minStay}, price=${d.price}, avail=${d.availability}`);
      });
    }
  });
}
