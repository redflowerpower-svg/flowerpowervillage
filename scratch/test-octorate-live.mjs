...
async function test() {
  ...
  const res = await fetch('https://api.octorate.com/connect/rest/v1/reservation?structure=366879', {
    headers: {
  ...
  console.log('Totale prenotazioni trovate da Octorate API live:', items.length);
  items.forEach(r => {
    console.log(`- ${r.guestName || r.firstName + ' ' + r.lastName} | Camera: ${r.roomName} (Product ID: ${r.product}) | ${r.checkin?.slice(0, 10)} -> ${r.checkout?.slice(0, 10)} | Canale: ${r.channelName}`);
  });
}
