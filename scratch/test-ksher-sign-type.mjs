...
async function testSignType(label, signTypeVal) {
  const params = {
    appid: appId,
    channel_list: 'card',
    fee_type: 'THB',
  ...
  try {
    ...
    console.log(`[sign_type: ${signTypeVal}] -> code: ${data.code}, msg: ${data.msg || data.message}`);
    if (data.code === 0 && data.data?.pay_url) {
      console.log('🎉🎉🎉 SUCCESS PAY URL:', data.data.pay_url);
    }
  } catch (err) {
    console.log(`[sign_type: ${signTypeVal}] ERR:`, err.message);
  }
}

async function run() {
  await testSignType('RSA', 'RSA');
  await testSignType('SHA256', 'SHA256');
  await testSignType('MD5', 'MD5');
}
