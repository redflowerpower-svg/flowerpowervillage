...
async function debugPenthouseCalc() {
  ...
  while (hasMore && page <= 10) {
    ...
    if (octRes.ok) {
      ...
      if (pageData.length > 0) {
        ...
        if (page + 1 >= totalPages || pageData.length < 200) hasMore = false;
        else page++;
      } else hasMore = false;
    } else hasMore = false;
  }
  ...
  const penthouseBookings = [];
  activeBookings.forEach(b => {
    const c = getCanonicalAccommodation(b);
    if (c && c.key === 'villa penthouse') {
      ...
      penthouseBookings.push({ in: inStr, out: outStr, id: b.id, name: b.guestName || b.firstName, rawIn, rawOut });
    }
  });

  console.log(`Found ${penthouseBookings.length} active bookings for Penthouse:`);
  penthouseBookings.sort((a,b) => a.in.localeCompare(b.in)).forEach(b => {
    console.log(` - ID: ${b.id} | ${b.in} -> ${b.out} (${b.name}) | rawIn: ${b.rawIn}, rawOut: ${b.rawOut}`);
  });

  // Calculate gaps
  const sorted = penthouseBookings.sort((a, b) => a.in.localeCompare(b.in));
  ...
  gaps.filter(g => g.start >= '2026-12-01' && g.start <= '2027-01-31').forEach(g => {
    console.log(` - Gap from ${g.start} to ${g.end} (${g.days} days) -> blockMinStay will be: ${Math.min(g.days, getBaselineMinStay(g.start))}`);
  });
}
