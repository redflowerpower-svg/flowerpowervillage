...
async function checkCalendar() {
  ...
  let allItems = [];
  while (page < 15) {
    const url = `https://api.octorate.com/connect/rest/v1/calendar/${structureId}?dateFrom=2026-12-28&dateTo=2027-01-06&size=20&page=${page}`;
  ...
  const targetRates = [421511, 449348, 921799, 932256];
  ...
  matched.forEach(r => {
    ...
    if (r.days) {
      r.days.forEach(d => {
        console.log(`   ${d.date}: minStay=${d.minStay}, price=${d.price}, stopSells=${d.stopSells}`);
      });
    }
  });
}
