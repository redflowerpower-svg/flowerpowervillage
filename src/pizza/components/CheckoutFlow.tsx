...
export default function CheckoutFlow({ onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Upload file solo se è PromptPay e abbiamo un file
      ...
      console.error(err);
      alert('Errore durante l\'invio dell\'ordine: ' + (err instanceof Error ? err.message : 'Controlla database'));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-stone-900 w-full max-w-md p-6 border border-stone-800 text-white relative rounded-lg shadow-2xl">
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <button type="button" onClick={requestLocation} className="w-full bg-stone-800 p-2 text-sm flex items-center justify-center gap-2 hover:bg-stone-700 transition-colors">
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <button onClick={handleSubmit} className="w-full bg-green-700 p-3 rounded font-bold hover:bg-green-600 transition-colors" disabled={loading}>
