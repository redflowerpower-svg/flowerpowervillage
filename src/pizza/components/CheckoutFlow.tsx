...
export default function CheckoutFlow({ onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async () => {
    // Controllo obbligatorio: se PromptPay, serve la ricevuta
    if (paymentMethod === 'promptpay' && !receiptFile) {
      alert('Per favore, carica la ricevuta di pagamento per completare l\'ordine con PromptPay.');
      return;
    }
      ...
      <div className="bg-stone-900 w-full max-w-md p-6 border border-stone-800 text-white relative rounded-lg">
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <button onClick={handleSubmit} className="w-full bg-green-700 p-3 font-bold rounded" disabled={loading}>
