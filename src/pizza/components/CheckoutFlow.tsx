...
export default function CheckoutFlow({ onClose, onSuccess }: Props) {
  ...
  const { requestLocation, setSimulatedLocation, isLoading: locationLoading, distanceKm, error: locationError } = useLocationStore();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (paymentMethod === 'promptpay' && receiptUrl) {
      ...
      <div className="bg-stone-900 w-full max-w-md p-6 border border-stone-800 text-white relative rounded-lg shadow-2xl">
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            {/* Controllo distanza 6km */}
            {distanceKm !== null && distanceKm > 6 && (
              <div className="bg-red-900/50 p-3 rounded border border-red-700 text-center">
                <p className="text-red-200 text-sm font-bold">
                  ...
                  Consegniamo fino a un massimo di 6 km.
                </p>
                <button onClick={setSimulatedLocation} className="text-[10px] text-red-400 underline mt-2">Simula posizione (Test)</button>
              </div>
            )}

            <button 
              onClick={() => setStep(2)} 
              disabled={distanceKm !== null && distanceKm > 6}
