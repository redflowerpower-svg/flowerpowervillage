...
export default function CheckoutFlow({ onClose, onSuccess }: Props) {
  ...
  const [address, setAddress] = useState(''); // Indirizzo personalizzato
      ...
      <div className="bg-stone-900 w-full max-w-md p-6 border border-stone-800 text-white relative">
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            {paymentMethod === 'promptpay' && (
              <div className="bg-white p-4 flex justify-center"><img src={QR_URL} alt="QR" className="w-48" /></div>
            )}
        ...
        {step === 3 && (
          <div className="text-center py-8 space-y-4">
            ...
            <button onClick={onSuccess} className="bg-stone-700 p-3 w-full">Torna al Menu</button>
          </div>
        )}
      </div>
    </div>
